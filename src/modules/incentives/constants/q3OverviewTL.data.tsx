...
export const buildQ3TLCarryForward = (data?: EmployeeIncentiveData | null) => ({
  title: "Q3 · Shortfall carry-forward active",

  description: `Q2 shortfall of ${data?.carryForwardMultiple ?? 0}x carried forward. Required: ${
  ...
  q3Minimum: `${data?.reqTeamRevMultiple ?? 3}.0x`,

  q2Shortfall: `${data?.carryForwardMultiple ?? 0}x`,
...

    {
      ...
      subtitle:
        data?.newAccountsIncentive != null && data.newAccountsIncentive > 0
          ? `Incl. ₹${data.newAccountsIncentive.toLocaleString(
              "en-IN",
            )} acquisition bonus`
...
export const buildQ3TLSelfMetrics = (
  data?: EmployeeIncentiveData | null,
): MetricCardData[] => [
  {
    id: "self-broking",
...
export const buildQ3TLSelfCriteria = (data?: EmployeeIncentiveData | null) => ({
...
export const buildQ3TLTeamMetrics = (
  data?: EmployeeIncentiveData | null,
): MetricCardData[] => [
  {
    id: "team-broking",
...
export const buildQ3TLTeamCriteria = (data?: EmployeeIncentiveData | null) => ({
...
export const buildQ3TLEligibility = (
  data?: EmployeeIncentiveData | null,
): TeamEligibilityChecklistData => {
  const items: TeamEligibilityChecklistData["items"] = [
...

    {
      id: "2",
      label: `Team min ${
        data?.reqTeamRevMultipleWithCarry ?? 3
      }x CTC (${data?.reqTeamRevMultiple ?? 3}x + ${
        data?.carryForwardMultiple ?? 0
      }x carried)`,
...

    {
      id: "4",
      label: `Team min ${
        data?.reqTeamNonBrokRevMultiple ?? 1
      }x from Non-broking`,
...
export const Q3_TL_ROLE: TeamRoleData = {
  title: "Team Leader",
